...
# We separate "Title Keywords" from "Description Keywords" to avoid pollution
...
def fetch_job_market_snapshot():
    ...
    for category, keywords in SEARCH_MAP.items():
        # Using the 'what' parameter ensures the API does the heavy lifting
...
# --- 3. CATEGORIZATION (THE FIX: TITLE PRIORITY) ---
def apply_negative_logic_tagging(raw_jobs):
    ...
    for job in raw_jobs:
        ...
        # --- THE FIX: STEP 1 - ELIMINATE MISMATCHES ---
        # If 'mechanical' or 'civil' is in the title, it IS Engineering, even if it mentions 'design'
        ...
        # --- STEP 2 - STRICT GREEN MATCHING ---
        # Only tag Green if the title actually suggests a green role
        ...
        # --- STEP 3 - STRICT DESIGN MATCHING ---
        # Exclude "Engineers" and "Sales" from Design
        ...
        # --- STEP 4 - FALLBACK TO OTHER CATEGORIES ---
...
def run_report(tagged_jobs):
    ...
    summary = df.groupby('assigned_category').agg({'days_on_market': 'mean', 'title': 'count'}).rename(columns={'title': 'vacancies', 'days_on_market': 'friction'})
    all_themes = list(SEARCH_MAP.keys())
    summary = summary.reindex(all_themes).fillna(0)
    
    demand_targets = {'Green': 95, 'Software/Tech': 70, 'Design': 45, 'Engineering': 80, 'Construction': 60, 'Education': 50, 'Health': 85, 'Finance': 45, 'Legal': 40, 'Legacy (Tourism/Retail)': 30}
    ...
    for cat in all_themes:
        ...
        if cat_df.empty:
            ...
            for _, row in cat_df.iterrows():
                ...
                ax2.text(0.02, report_y, f"{days:2d}d | {title:<68} | {str(row.get('company', {}).get('display_name'))[:25]}", fontsize=9, family='monospace', transform=ax2.transAxes)
                report_y -= 0.012
        report_y -= 0.01

    plt.tight_layout()
    filename = 'bristol_cleaned_report.png'
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    
    if sys.platform == "darwin": subprocess.run(['open', filename])
    elif sys.platform == "win32": os.startfile(filename)
    else: subprocess.run(['xdg-open', filename])
